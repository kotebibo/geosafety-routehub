...
export function useBoardHandlers({
  ...
  const handleCellEdit = useCallback(
    async (rowId: string, columnId: string, value: any) => {
      try {
        ...
        const oldValue = columnId === 'name' ? currentItem.name : currentItem.data?.[columnId]
  ...
  const handleUpdateColumn = useCallback(
    async (columnId: string, updates: Partial<BoardColumn>) => {
      try {
  ...
  const handleReorderColumns = useCallback(
    async (reorderedColumns: BoardColumn[]) => {
      try {
  ...
  const handleColumnConfigUpdate = useCallback(
    async (columnId: string, config: Record<string, any>) => {
      ...
      if (previous) {
        queryClient.setQueryData(
          columnsQueryKey,
          previous.map(col => (col.id === columnId ? { ...col, config } : col))
        )
      }
  ...
  const handleTableColumnReorder = useCallback(
  ...
  const handleGroupCollapseToggle = useCallback(
    async (groupId: string, isCollapsed: boolean) => {
      // Virtual groups (from dynamic grouping) can't be persisted
      if (groupId.startsWith('groupby_')) return
      try {
  ...
  const handleDeleteGroup = useCallback(
    async (groupId: string) => {
      // Cannot delete virtual groups
      if (groupId.startsWith('groupby_')) {
        showToast('Cannot delete a dynamic group', 'error')
        return
      }
      ...
      try {
        // Check both group_id column AND data.group_id for items in this group
        const itemsInGroup =
          items?.filter(item => item.group_id === groupId || item.data?.group_id === groupId) || []
  ...
  const handleItemMove = useCallback(
    async (itemId: string, targetGroupId: string) => {
      // Cannot move items to virtual groups — they are ephemeral
      if (targetGroupId.startsWith('groupby_')) {
        showToast('Cannot move items when dynamic grouping is active', 'warning')
        return
      }
  ...
  const handleItemReorder = useCallback(
    async (itemId: string, targetItemId: string, position: 'before' | 'after') => {
      try {
        if (!items) return
        ...
        // Cannot persist reorder within virtual groups
        if (targetGroupId.startsWith('groupby_')) {
          showToast('Cannot reorder items when dynamic grouping is active', 'warning')
          return
        }

...
export const MemoizedTableRow = memo(
  function MemoizedTableRow({
    ...
    const getUsersEditingCell = useCallback(
    ...
    return (
      ...
      >
        ...
        >
          {visibleColumns[0] && (
            <CellRenderer
              ...
              value={
                visibleColumns[0].column_id === 'name'
                  ? (item.name ?? 'Unnamed')
        ...
        {visibleColumns.slice(1).map((column, colIndex) => {
          ...
          const isCellInEditMode = isCellEditing(
            globalRowIndex,
            columnIndex,
            editingRowIndex,
            editingColumnIndex
        ...
        <td
          className={cn('border border-[#c3c6d4] p-0', isSelected ? 'bg-[#e5e9ff]' : 'bg-white')}
  ...
  (prevProps, nextProps) => {
    ...
    if (prevProps.isDragOver && prevProps.dragOverPosition !== nextProps.dragOverPosition)
      return false
    ...
    if (
      prevFocusedOnThisRow &&
      nextFocusedOnThisRow &&
      prevProps.focusedColumnIndex !== nextProps.focusedColumnIndex
    )
      return false
    ...
    if (
      prevEditingOnThisRow &&
      nextEditingOnThisRow &&
      prevProps.editingColumnIndex !== nextProps.editingColumnIndex
    )
      return false
